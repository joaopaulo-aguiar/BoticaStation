...
function tokenize(template: string): Token[] {
  ...
  while ((match = tagRegex.exec(template)) !== null) {
    ...
    if (tag === '#each') tokens.push({ type: 'each_start', path: param })
    else if (tag === '/each') tokens.push({ type: 'each_end' })
    else if (tag === '#if') tokens.push({ type: 'if_start', path: param })
    else if (tag === '/if') tokens.push({ type: 'if_end' })
    else if (tag === '#unless') tokens.push({ type: 'unless_start', path: param })
    else if (tag === '/unless') tokens.push({ type: 'unless_end' })
    else if (tag === 'else') tokens.push({ type: 'else' })
    else tokens.push({ type: 'variable', path: tag })
...
function parseTokens(tokens: Token[]): ASTNode[] {
  ...
  function parseUntil(endCondition: (token: Token) => boolean): ASTNode[] {
    ...
    while (i < tokens.length) {
      ...
      if (token.type === 'text') { result.push({ type: 'text', value: token.value }); i++ }
      else if (token.type === 'variable') { result.push({ type: 'variable', path: token.path }); i++ }
  ...
  for (const node of nodes) {
    if (node.type === 'text') {
      ...
      result += isTruthy(value)
        ? renderNodes(node.thenBranch, context, rootData)
        : renderNodes(node.elseBranch, context, rootData)
